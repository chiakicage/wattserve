...
try:
    from .render_llama_component_ablation_report import (
        ROOT_COMPONENT_ABLATION_INDEX_PATH,
        render_result_report,
    )
except ImportError:
    from render_llama_component_ablation_report import (  # type: ignore[no-redef]
        ROOT_COMPONENT_ABLATION_INDEX_PATH,
        render_result_report,
    )
...
DEFAULT_RESULTS_ROOT = (
    REPO_ROOT / "results" / "llama_component_ablation_prefill"
)
...
def run_llama_component_ablation_matrix(
    output_dir: Path,
    warmup: int,
    repeat: int,
    ...
    metadata: dict[str, Any] = {
        ...
        "variants": [variant["variant"] for variant in DEFAULT_VARIANTS],
        "variant_definitions": DEFAULT_VARIANTS,
        "warmup": warmup,
        "repeat": repeat,
    ...
    for model in DEFAULT_MODELS:
        for prompt_len in DEFAULT_PROMPT_LENGTHS:
            for variant in DEFAULT_VARIANTS:
                ...
                variant_flags = {
                    flag_name: bool(variant[flag_name])
                    for flag_name in FLAG_FIELDNAMES
                }
                monitor_csv_path = (
                    monitor_dir / f"{model}_prompt{prompt_len}_{variant_name}.csv"
                )
                print(
                    "Running "
                    f"model={model} prompt_len={prompt_len} variant={variant_name}",
                    flush=True,
                )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the Llama component ablation benchmark matrix."
    )
    parser.add_argument(
        "--output_dir",
        type=Path,
    ...
    parser.add_argument(
        "--skip_root_index",
        action="store_true",
        help="Do not refresh the repo-root BENCHMARK_COMPONENT_ABLATION.md index.",
    )
    return parser
...
def main() -> int:
    ...
    run_llama_component_ablation_matrix(
        output_dir=output_dir,
        warmup=args.warmup,
        repeat=args.repeat,
